...
static void icc_button_clicked    (IccButton      *button,
                                   gpointer        data);
void icc_button_run_dialog (GtkWidget      *widget,
                                   gpointer        data);
                               ...
                               gpointer          data)
{
  ...
  gboolean do_freev = TRUE;

  //uris = gtk_selection_data_get_uris(selection_data)
  uris = g_uri_list_extract_uris (
	  (const gchar*)gtk_selection_data_get_uris(selection_data));
  if(uris[0] == NULL)
  {
	  uris = g_malloc(sizeof(*uris) * 2);
	  (void)memset(uris, 0, sizeof(*uris) * 2);
	  uris[0] = (gchar*)gtk_selection_data_get_data(selection_data);;
	  do_freev = FALSE;
  }
...
profile_data_remove_filename (const gchar *filename)
{
  ...
  for (i = 0; i < (gint)profileDataArray->len; i++)
    {
              ...
              if (profile_data_new_from_file (tmp, &data, ICC_BUTTON_CLASS_ALL, ICC_BUTTON_COLORSPACE_ALL, ICC_BUTTON_COLORSPACE_ALL))
                {
                  ...
                  if (i >= (gint)profileDataArray->len)
                    {
...
searchProfile (void)
{
  ...
  {
#if defined _WIN32 // #if defined G_OS_WIN32
  DWORD DirNameSize;
  int j = 3;

#ifdef _WIN64
  gchar *searchPath[6];
  searchPath[j++] = g_build_filename (g_getenv ("COMMONPROGRAMFILES(X86)"), "\\adobe\\color\\profiles", NULL);;
#else
  gchar *searchPath[5];
#endif
  searchPath[j++] = g_build_filename (g_getenv ("COMMONPROGRAMFILES"), "\\adobe\\color\\profiles", NULL);
  GetColorDirectory (NULL, NULL, &DirNameSize);
  searchPath[j] = g_try_malloc0 (DirNameSize);
  GetColorDirectory (NULL, searchPath[j], &DirNameSize);
  ...
  if (--nInstances <= 0)
    {
      ...
      for (i = count = 0; i < (gint)profileDataArray->len; i++)
        {
          profileData *data;
          gchar *tmp;
...
setupMenu (IccButton *button)
{
  ...
  for (i = button->nEntries = 0; i < (gint)profileDataArray->len; i++)
    {
      profileData *data = g_array_index (profileDataArray, profileData *, i);
  ...
  if (n_menu_items > 1)
    {
      ...
      item = gtk_separator_menu_item_new();
      gtk_menu_shell_append( GTK_MENU_SHELL (button->popupMenu), item);
      item = gtk_menu_item_new_with_label ((gint)button->menuItems->len < button->nEntries ?
                          ...
                          gpointer  user_data)
{
  ...
  gdk_window_get_origin(gtk_widget_get_window(widget), &_x, &_y);

  gtk_widget_get_allocation(widget, &allocation);
  if (allocation.width > button->menuWidth)
    {
  ...
  else
    {
      ...
      _x += allocation.x - (button->menuWidth - allocation.width) / 2;
    }
  ...
  _y += allocation.y - allocation.height / 2 + 2;
  ...
  if (button->popupMenu && button->menuItems->len > 0)
    {
      ...
      if (!button->path)
      ...
      else
        {
          ...
          for (i = 0; i < (gint)button->menuItems->len; i++)
            {
                       ...
                       gpointer   data)
{
  ...
  gtk_window_set_transient_for(GTK_WINDOW(button->dialog), NULL);

  hbox = gtk_hbox_new (FALSE, 8);
  gtk_container_set_border_width (GTK_CONTAINER (hbox), 8);
  gtk_box_pack_start (GTK_BOX (
	  gtk_dialog_get_content_area(GTK_DIALOG (button->dialog))), hbox, TRUE, TRUE, 0);
  ...
  gtk_box_pack_start (GTK_BOX (gtk_dialog_get_content_area(
	  GTK_DIALOG (button->dialog))), hbox, FALSE, FALSE, 0);
