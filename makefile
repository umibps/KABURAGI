CC		= gcc
CPP		= g++
CFLAGS		= `pkg-config --cflags bullet tbb gtk+-2.0 gtkglext-1.0 assimp` -O3 -w -fopenmp
...
$(PLUG_INS):
		make -f ./plug-in-src/effect_plug-in/makefile
		cp ./plug-in/effect_plug-in.so ./plug-in-src/effect_plug-in/effect_plug-in.so
		make -f ./plug-in-src/mine_sweeper/makefile
		cp ./plug-in/mine_sweeper.so ./plug-in-src/mine_sweeper/mine_sweeper.so
		make -f ./plug-in-src/plug-in_brush/makefile
		cp ./plug-in/plug-in_brush.so ./plug-in-src/plug-in_brush/plug-in_brush.so
		make -f ./plug-in-src/texturize/makefile
		cp ./plug-in/texturize.so ./plug-in-src/texturize/texturize.so
		make -f ./plug-in-src/kakeami/makefile
		cp ./plug-in/kakeami.so ./plug-in-src/kakeami/kakeami.so
		make -f ./plug-in-src/saturated_linework/makefile
		cp ./plug-in/saturated_linework.so ./plug-in-src/saturated_linework/saturated_linework.so
