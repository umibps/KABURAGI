...
Win32WarningHandler(const char* module, const char* fmt, va_list ap)
{
	...
	LPTSTR szTitle;
	LPTSTR szTmp;
	LPCTSTR szTitleText = (LPCTSTR)"%s Warning";
	LPCTSTR szDefaultModule = (LPCTSTR)"LIBTIFF";
	LPCTSTR szTmpModule = (module == NULL) ? szDefaultModule : (LPCTSTR)module;
        SIZE_T nBufSize = (strlen((const char*)szTmpModule) +
                        strlen((const char*)szTitleText) + strlen(fmt) + 256)*sizeof(char);

	if ((szTitle = (LPTSTR)LocalAlloc(LMEM_FIXED, nBufSize)) == NULL)
		return;
	sprintf((char*)szTitle, (const char*)szTitleText, szTmpModule);
	szTmp = szTitle + (strlen((const char*)szTitle)+2)*sizeof(char);
	vsnprintf((char*)szTmp, nBufSize-(strlen((const char*)szTitle)+2)*sizeof(char), fmt, ap);
	MessageBoxA(GetFocus(), (LPCSTR)szTmp, (LPCSTR)szTitle, MB_OK | MB_ICONINFORMATION);
	LocalFree(szTitle);
...
Win32ErrorHandler(const char* module, const char* fmt, va_list ap)
{
	...
	LPTSTR szTitle;
	LPTSTR szTmp;
	LPCTSTR szTitleText = (LPCTSTR)"%s Error";
	LPCTSTR szDefaultModule = (LPCTSTR)"LIBTIFF";
	LPCTSTR szTmpModule = (module == NULL) ? szDefaultModule : (LPCTSTR)module;
        SIZE_T nBufSize = (strlen((const char*)szTmpModule) +
                        strlen((const char*)szTitleText) + strlen(fmt) + 256)*sizeof(char);
	...
	szTmp = szTitle + (strlen((const char*)szTitle)+2)*sizeof(char);
	vsnprintf((char*)szTmp, nBufSize-(strlen((const char*)szTitle)+2)*sizeof(char), fmt, ap);
	MessageBoxA(GetFocus(), (LPCSTR)szTmp, (LPCSTR)szTitle, MB_OK | MB_ICONEXCLAMATION);
	LocalFree(szTitle);
	return;
#else
	if (module != NULL)
