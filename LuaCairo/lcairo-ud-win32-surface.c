...
static int luaopen_lWin32Surface (lua_State *L)
{
	...
	luaL_setfuncs(L, Win32Surface_lib_m, 0);
    //luaL_register(L, NULL, Win32Surface_lib_m);
	luaL_newlib(L, Win32Surface_lib_f);
	lua_setglobal(L, "Win32Surface");
...
static int luaopen_lWin32PrintingSurface (lua_State *L)
{
    ...
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
	luaL_newlib(L, Win32PrintingSurface_lib_m);
    //luaL_register(L, NULL, Win32PrintingSurface_lib_m);
	luaL_newlib(L, Win32PrintingSurface_lib_f);
	lua_setglobal(L, "Win32PrintingSurface");
