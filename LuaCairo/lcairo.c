...
static void set_functions(lua_State* L, const struct luaL_Reg* reg)
{
	luaL_Reg *reg_data;
	luaL_Reg *start_reg = reg;
	int num_data = 0;
	int i;
	...
	reg_data = (luaL_Reg*)malloc(sizeof(*reg_data)*num_data);

	for(i=0; i<num_data; i++)
	{
		reg_data[i].name = reg[i].name;
		reg_data[i].func = reg[i].func;
	}

	lua_createtable(L, 0, num_data-1);
	luaL_setfuncs(L, reg_data, 0);
