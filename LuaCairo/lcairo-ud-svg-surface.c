...
static int new_SvgSurface (lua_State *L)
{
    ...
    cs = cairo_svg_surface_create(filename, width_in_points, height_in_points);
...
static int luaopen_lSvgSurface (lua_State *L)
{
	...
	luaL_newlib(L, SvgSurface_lib_m);
    //luaL_register(L, NULL, SvgSurface_lib_m);
	luaL_newlib(L, SvgSurface_lib_f);
	lua_setglobal(L, "SvgSurface");
