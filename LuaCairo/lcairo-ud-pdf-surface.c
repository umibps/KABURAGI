...
static int new_PdfSurface (lua_State *L)
{
    ...
    cs = cairo_pdf_surface_create(filename, width_in_points, height_in_points);
...
static int luaopen_lPdfSurface (lua_State *L)
{
	...
	luaL_newlib(L, PdfSurface_lib_m);
    //luaL_register(L, NULL, PdfSurface_lib_m);
	luaL_newlib(L, PdfSurface_lib_f);
	lua_setglobal(L, "PdfSurface");
